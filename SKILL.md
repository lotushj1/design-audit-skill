---
name: design-audit
description: 平面設計稽核工具（design audit / pre-delivery check）。當使用者提供社群圖卡、廣告圖、活動海報、簡報封面、輪播圖、產品宣傳圖，並要求檢查問題、找錯、交付前檢查、設計稽核、確認能不能交付、檢查錯字/中國用語/廣告風險，或以英文要求 review/check/audit 這類圖像時，必須使用此 skill。此 skill 不是設計拆解，也不是主觀美感建議；它會列出所有可觀察問題、位置、判定依據與非數值改善方向。
---
...
收到不適用類型的作品時，先說明此 skill 的稽核範圍與不適用原因，不要硬套 10 個檢查目標；若畫面中仍有屬於適用範圍的平面元素（例如簡報中的單張圖卡頁），只稽核該部分並說明範圍。
...
- Critical：會讓主訊息失效、造成誤解、關鍵文字不可讀、CTA 失效、重要資訊錯誤、核心內容被裁切，或有高廣告風險。不建議交付。
- High：明顯影響閱讀、理解、轉換、專業感，或輸出邊界有白邊、異色條、透明殘留等可見輸出瑕疵。建議優先修改。
...
多圖時，每張的「文字與廣告風險提醒」放在該張區塊內；該張沒有風險就省略該區塊。
